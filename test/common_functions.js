...
function storageAssert(storage, admin, tokensaleStatus, totalTokenAmount, totalBaseAssetAmount, fa12Address, closeDate, tokenWeight, baseAssetName, tokenDecimals, tokenName, basedAssetAddress) {
    assert(storage.admin == admin, `admin: ${storage.admin}; expected: ${admin}`);
    let token_list = storage.token_list;
    assert(token_list.sale == tokensaleStatus, `tokensale status: ${token_list.sale}; expected: ${tokensaleStatus}`)
    assert(equal(token_list.token_amount, toFloat(totalTokenAmount)), `total_token_amount: ${token_list.token_amount}; expected: ${toFloat(totalTokenAmount)}`)
    ...
    assert(token_list.token_address == fa12Address, `token address: ${token_list.token_address}; expected: ${fa12Address}`)
    ...
    assert(token_list.based_asset_name == baseAssetName, `based_asset_name: ${token_list.based_asset_name}; expected: ${baseAssetName}`)
    assert(token_list.token_dec == tokenDecimals, `token_dec: ${token_list.token_dec}; expected: ${tokenDecimals}`)
    assert(token_list.token_name == tokenName, `token_name: ${token_list.token_name}; expected: ${tokenName}`)
    assert(token_list.based_asset_address == basedAssetAddress, `based_asset_address: ${token_list.based_asset_address}; expected: ${basedAssetAddress}`)
}
